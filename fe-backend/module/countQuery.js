...
function getKeyVal(req, res) {
  ...
  console.log("final matchQuery ",matchQuery);
  ...
  Keywords.aggregate(
    [
      ...
      // { $addFields : { keywords : }},
      {
        $project: {
          count: {
            $slice: [ "$count", { $min: [ { $size: "$count" }, num ] } ], //5번째 elemnt(왼쪽 param)까지 3개만큼(right param)
          },
        },
      },
      {
...
function getTopKeywords(req, res) {
...
router.post("/getTopKeywords", getTopKeywords);
